...
impl From<(ErrorKind, &'static str, String)> for RedisError {
	fn from(
		(kind, desc, detail): (ErrorKind, &'static str, String),
	) -> RedisError {
		RedisError {
		...
		match self.repr {
			ErrorRepr::IoError(ref err) => {
				match err.kind() {
					...
					// connection refusal.
					io::ErrorKind::NotFound => cfg!(feature = "uds"),
					_ => false,
				}
			}
			_ => false,
	...
	($t:ty, $v:expr) => {{
		...
		match *v {
			...
			Value::Status(ref s) => match s.parse::<$t>() {
				Ok(rv) => Ok(rv),
				Err(_) => {
					invalid_type_error!(v, "Could not convert from string.")
				}
			},
...
impl<K: FromRedisValue + Eq + Hash, V: FromRedisValue> FromRedisValue
	for HashMap<K, V>
{
	...
	K: Ord,
{
	fn from_redis_value(v: &Value) -> RedisResult<BTreeMap<K, V>> {
		match *v {
			...
			_ => {
				invalid_type_error!(v, "Response type not btreemap compatible")
			}
		}
	}
}
	...
	T: Ord,
{
	fn from_redis_value(v: &Value) -> RedisResult<BTreeSet<T>> {
		match *v {
			...
			_ => {
				invalid_type_error!(v, "Response type not btreeset compatible")
			}
		}
	}
}
